...
--1. 创建带有分区的特定格式的表（可以根据时间不断向表格里添加数据）
...
--2. 创建完成功后，需要向表中添加数据
--基本格式：
insert overwrite table ** partition (ds = '**')
    select  XX ,count(XXX) as  from  XXX   where  pt =' ';   --覆盖数据
--或者：
insert intotable **  partition（ds ='**'）
...
--4. 如何通过hive进行数据统计
select id , count(xx) as c from xxx where pt = ' ' group by id sort by c desc ;
...
select t.*, row_number() over(partition by t.category order by t.auction_count desc ) as rank from
    (select category, auction_id, count(user_id) as auction_count from xxx
        where ds ='xxxx'
        group by category, auction_id) t
    where rank<=10;
...
--7. coalesce
COALESCE( value1,value2,... ) --返回list中第一个不是null的值
...
--1. 统计：乔阳 在７月16日以后每天与多少个卖家发生了交易，并按照交易笔数对每天的卖家进行倒序排列。
...
--2. 多个条件匹配，要怎么写呢？验证8月1日和8月2日，昵称带有'乔阳'两个字的用户在掌上旺信或者湖畔上有登录过。
...
--3. 如何给特定的字段添加新的内容呢？要8月1日注册的用户在昵称后面统一添加cntaobao的标签。
...
--7. 统计每个action_type_id下有多少个不同的from_user_id：
